# python run_answer_quality_avg.py --obj_name car --base_name jackson-town-square \
...
def main():
    ...
    print("This code is to compute the answer quality of AVG under different interventions")
    print('True length {}, true average {}'.format(len(Y_val)*ground_truth_freq, np.sum(Y_val)/(len(Y_val)*ground_truth_freq)))
    print('predict average {}'.format(np.sum(Y_pred)/(len(Y_pred)*ground_truth_freq)))
    conf = 0.05
    nb_trials = 100 
    ...
    for i in range(nb_trials):
        ...
        true_err = np.abs(estimate - np.sum(Y_val)/(len(Y_val)*ground_truth_freq))/(np.sum(Y_val)/(len(Y_val)*ground_truth_freq))
