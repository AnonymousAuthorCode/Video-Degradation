...
def performBatchDetect(thresh= 0.25, configPath = "./cfg/yolov4.cfg", weightPath = "yolov4.weights", metaPath= "./cfg/coco.data", hier_thresh=.5, nms=.45, batch_size=3):
    ...
    img_samples = ['data/person.jpg', 'data/eagle.jpg', 'data/dog2.jpg']
...
if __name__ == "__main__":
    # mkdir + edit resolution in yolov4.cfg + edit csv name in this code
    detect_result = []
    sample_path = "../../Data/sample_frames/UA-DETRAC/freq1_resori/"
    # sample_path = "../../Data/sample_frames/UA-DETRAC_share/MVI_40775/"

    start_time = time.time()
    for index in np.arange(0,15210):
        detections = performDetect(imagePath=sample_path+"%d.jpg"%index, thresh= 0.7, showImage=False)
        # detections = detections['detections']
        for i in range(len(detections)):
            if(detections[i][0] == 'car' or detections[i][0] == 'person'):
                detect_result.append([index, detections[i][0]])

    # start_time = time.time()
    # for imgdir in os.listdir(sample_path):
    #     detections = performDetect(imagePath=sample_path+imgdir, thresh=0.7, showImage=False)
    #     index = int(imgdir[3:8])-1
    #     # detections = detections['detections']
    #     for i in range(len(detections)):
    #         if(detections[i][0] == 'car' or detections[i][0] == 'person'):
    #             detect_result.append([index, detections[i][0]])
    ...
    df.to_csv("../../Data/filtered/UA-DETRAC/freq1_res64_yolo/UA-DETRAC-2020-09-19.csv", index=None)
    # df.to_csv("../../Data/filtered/UA-DETRAC_share/40775_res128_yolo/UA-DETRAC_share-2020-09-19.csv", index=None)
